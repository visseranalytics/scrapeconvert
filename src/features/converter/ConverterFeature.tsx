...
import { ImageFile, ConversionSettings, ConversionFormat } from '@/shared/types';
import { readFileAsDataURL, getImageDimensions, convertImage, getExtensionFromMimeType, formatBytes } from '@/shared/services/imageUtils';
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
              <div className="flex flex-col gap-1">
                <h3 className="text-xl font-bold text-white flex items-center gap-2">
                  Ready to Convert
                  <span className="text-sm font-normal text-slate-500 bg-slate-800 px-2 py-0.5 rounded-full">{files.length} {files.length === 1 ? 'image' : 'images'}</span>
                </h3>
                {hasSuccessfulConversions ? (
                  ...
                  <p className="text-xs sm:text-sm text-slate-400">
                    Total size: <span className="text-slate-200 font-mono">{formatBytes(totalOriginalSize)}</span>
                  </p>
                )}
              </div>
